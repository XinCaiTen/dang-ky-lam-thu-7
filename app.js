...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        ...
        loadData().then(() => {
            pageLoader.nextStep(); // Bước 4: Khởi tạo giao diện
            loadRealtimeHistory();
            
            // Ẩn loader sau khi tất cả đã sẵn sàng
            setTimeout(() => {
                pageLoader.hide();
            }, 500);
        });
        ...
        // Ẩn loader cho màn hình login
        pageLoader.nextStep(); // Bước 4
        setTimeout(() => {
            pageLoader.hide();
        }, 500);
    }
...
window.updateStatus = async function(userId, type, checked) {
    try {
        const dateStr = formatDateForAPI(currentSaturday);
